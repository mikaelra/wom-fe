...
import { claimPendingArtifact } from '@/lib/api';
import { useClaimVerificationPoll } from '@/lib/useClaimVerificationPoll';

/**
 * Deliberately a near-copy of WheelClaimNudge rather than a shared,
 * parameterised nudge. The two differ in every string, and the claim they
 * gate is not equivalent: a missed Wheel is one of many, while an artifact
 * cannot be re-earned by anyone, ever. Folding them together would put that
 * asymmetry behind a prop and make it easy to lose.
 */
type Props = {
  lobbyId: string;
  playerName: string;
...
export default function ArtifactClaimNudge({ lobbyId, playerName, onDismiss }: Props) {
  ...
  const handleClaim = async () => {
    ...
    setLoading(true);
    try {
      const result = await claimPendingArtifact(lobbyId, playerName, trimmedEmail);
        ...
        {claimed ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-300 mb-4 text-center">
              Connect an email to your name and claim it. An artifact cannot be
              found twice, so this is the only chance to keep it.
            </p>
            <input
              type="email"
            ...
            <div className="flex gap-2 mt-1">
              ...
              >
                {loading ? 'Claiming…' : 'Create account and claim artifact'}
              </button>
              <button
                type="button"
                onClick={onDismiss}
