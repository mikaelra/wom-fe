    ...
    const fire = () => {
      ...
      const spawnBanner = (killer: string, pos: [number, number, number], atMs: number) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => setKillBanners((b) => b.filter((x) => x.id !== id)), 1716);
        }, atMs);
      };
          ...
          <SwordEffect
            ...
            onStrike={() => {
              if (ev.targetDefended && !ev.isIncoming) {
                ...
                const holdMs = (HOLD_DUR + postDurSec) * 1000 + 132; // buffer scaled to 0.66x
              ...
              if (ev.flashPosition) {
                ...
                setTimeout(() => setHitFlashEvents((s) => s.filter((x) => x.id !== fid)), 429); // scaled to 0.66x
              }
