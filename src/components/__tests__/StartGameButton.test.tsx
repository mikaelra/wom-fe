...
describe('StartGameButton', () => {
  it('is enabled and calls onStartGame when nothing has changed', () => {
    ...
    render(<StartGameButton state={baseState} btn="" onStartGame={onStartGame} />);
  ...
  it('does not block on the very first render, even though there is no prior snapshot to compare', () => {
    render(<StartGameButton state={baseState} btn="" onStartGame={vi.fn()} />);
  ...
  it('blocks for 5s after a player selection changes, then re-enables', () => {
    vi.useFakeTimers();
    const onStartGame = vi.fn();
    const { rerender } = render(<StartGameButton state={baseState} btn="" onStartGame={onStartGame} />);

    const changed: Player = { ...basePlayer, selected_relic_ids: [1] };
    act(() => {
      rerender(<StartGameButton state={{ ...baseState, players: [changed] }} btn="" onStartGame={onStartGame} />);
    });
  ...
  it('does not block again for an unrelated re-render with the same selections', () => {
    ...
    rerender(<StartGameButton state={{ ...baseState, round: 0 }} btn="" onStartGame={vi.fn()} />);
    expect(screen.getByText('Start Game')).not.toBeDisabled();
  });
  ...
  it('blocks for 5s when a player joins the lobby', () => {
    ...
    act(() => {
      rerender(<StartGameButton state={{ ...baseState, players: [basePlayer, bob] }} btn="" onStartGame={vi.fn()} />);
    });
  ...
  it('blocks for 5s when a player is kicked from the lobby', () => {
    ...
    const { rerender } = render(
      <StartGameButton state={{ ...baseState, players: [basePlayer, bob] }} btn="" onStartGame={vi.fn()} />
    );

    act(() => {
      rerender(<StartGameButton state={{ ...baseState, players: [basePlayer] }} btn="" onStartGame={vi.fn()} />);
    });
