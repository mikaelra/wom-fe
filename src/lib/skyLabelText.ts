 ...
 * `Sky` snapshot. Neither scene prints where a body stands (altitude and
 * compass point) or the Moon's lit fraction -- the sky itself shows both.
 *
 ...
 */
export function labelDetail(sky: Sky, body: AspectBody): string | null {
  const parts: string[] = [];
