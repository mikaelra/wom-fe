...
// A pre-rendered head-and-shoulders shot of each skin's actual .glb model
// (public/skins/thumbnails/, generated by render_thumb.mjs -- see that
// script's header for how to regenerate after a model changes). Static
// images rather than a live 3D preview per grid cell: the inventory grid
// can show up to a dozen of these at once, and browsers cap concurrent
// WebGL contexts (~8-16) -- a live canvas per card risks silently failing
// on exactly the accounts with the most skins.
export function skinThumbnailUrl(skinName: string): string {
  return `/skins/thumbnails/${skinName}.png`;
}
