...
| 0 | `APP_VERSION` / `BUILD_NUMBER` in CI | ✅ Done — merged wom-fe#296 2026-08-12 |
| 0 | `PROTOCOL_VERSION` + min-client + forced update | ✅ Version sent + backend rejection merged (wom-fe#296, wom-be#170); the blocking "Update required" screen with store deep links is still deferred to §5.4/§7 — nothing to link to on any store yet |
...
| 1 | Capacitor **iOS** shell + CORS origins | 🏗️ wom-fe#369 — `capacitor.config.ts` + `ios/` (SwiftPM, committed, landscape-locked). `capacitor://localhost` / `https://localhost` already in `wom-be/cors.py` `NATIVE_SHELL_ORIGINS`, so no backend change |
...
| 3 | fastlane match + macOS CI — **stand up early** (§7) | 🏗️ wom-fe#369 — `fastlane/` (`match` + `beta` + one-off `bootstrap_certs` lanes, App Store Connect API-key auth) and `.github/workflows/ios.yml` on `macos-15`. Runs unsigned-compile-check until the 7 Apple secrets + `IOS_TESTFLIGHT_ENABLED` var are set (secrets table in `ios/README.md`) |
